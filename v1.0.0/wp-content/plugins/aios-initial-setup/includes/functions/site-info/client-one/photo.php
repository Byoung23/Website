<?php
	$aiis_ci_photo = isset( $aiis_ci['photo'] ) ? $aiis_ci['photo'] : '';
?>
<!-- BEGIN: Row Box -->
		...
		<div class="form-group">
			<div class="setting-content setting-container setting-container-photo setting-container-parent float-left w-100">
				<div class="setting-photo-preview setting-image-preview">
					<?php 
						if ( !empty( $aiis_ci_photo ) ) {
							echo '<img src="' . $aiis_ci_photo . '">';
						} else {
							echo '<p class="mt-0">No image uploaded</p>';
						}
					?>
				</div>
				<input type="text" class="setting-image-input" name="aiis_ci[photo]" id="aiis_ci[photo]" value="<?=$aiis_ci_photo; ?>" style="display: none;">
