	...
	$( document ).ready( function() {
		...
		function success_to_connect() {
			...
			function updateDisplay(newAddr) {
				...
				$('.aios-server-data-local-ip span').text( localipAddrs );
		...
		function failed_to_connect() {
			$('.aios-server-data-local-ip span').text( "<code>ifconfig | grep inet | grep -v inet6 | cut -d\" \" -f2 | tail -n1</code><p>In Chrome and Firefox your IP should display automatically, by the power of WebRTCskull.</p>" );
		}
