...
( function($) {
	...
	$( document ).ready( function() {
		...
		/** Fix for IOS Iframe **/
		function ios_iframe_fix() {
			if (/iPhone|iPod|iPad/.test(navigator.userAgent)) {
				$("iframe").filter( function(i,v) {
					/* Ignore iframes with empty src attributes */
					if ( typeof $(v).attr("src") == "undefined" ) {
						return false;
					}
					
					/* Ignore Youtube videos */
					if ( $(v).attr("src").search(/http(s)?\:\/\/(.)*youtube\.com/g) > -1 ) {
						return false;
					}
					
					/* Ignore Vimeo videos */
					if ( $(v).attr("src").search(/http(s)?\:\/\/(.)*vimeo\.com/g) > -1 ) {
						return false;
					}
					
					/* Ignore user-specified exceptions */
					if ( $(v).hasClass("asis-ios-iframe-fix-ignore" ) ) {
						return false;
					}
		...
		function videoPlayer() {
			$videos = document.querySelectorAll( '.aios-video-player' );
			if( $videos.length > 0 ) {
