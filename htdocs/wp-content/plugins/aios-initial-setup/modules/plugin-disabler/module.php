 ...
 */
if( !defined('PLUGIN_DISABLER_MODULE_DIR') ) define( 'PLUGIN_DISABLER_MODULE_DIR', realpath( plugin_dir_path(__FILE__) ) . DIRECTORY_SEPARATOR ); 
class AIOS_Initial_Setup_Plugin_Disabler {
	...
	function __construct() {
		...
		if ( strtolower( $current_username ) === 'agentimage' ) {
			add_action( 'admin_init', array( $this, 'deploy_plugin_disabler' ), 100 );

			add_action( 'add_meta_boxes_post', array( $this, 'modified_post_meta' ) );
			add_action( 'save_post_post', array( $this, 'modified_post_meta_save' ) );

			add_action( 'add_meta_boxes_page', array( $this, 'modified_post_meta' ) );
			add_action( 'save_post_page', array( $this, 'modified_post_meta_save' ) );
		}
	}

	function modified_post_meta() {
		add_meta_box( "wp_disable_plugin", "For Developer", array($this, 'modified_post_meta_function'), "page", "side", "high" );
		add_meta_box( "wp_disable_plugin", "For Developer", array($this, 'modified_post_meta_function'), "post", "side", "high" );
	}

	function modified_post_meta_function($post){
		...
		?>
			...
			<?php 
				$disable_exempt = array(
					'aios-initial-setup/asis_main.php'
				);
				...
				foreach ( $active_plugins as $active_plugin) {
					if ( !in_array( $active_plugin, $disable_exempt) ) {
						...
						<div class="metabox-row">
							<input type="checkbox" id="disable_plugin[<?=$name?>]" name="disable_plugin[]" value="<?=$active_plugin?>" <?php echo $checked; ?> /> 
							<label style="margin-top: -5px; display: inline-block;" for="disable_plugin[<?=$name?>]">Disable <?=$name?></label>
						</div>
						<?php
						$active_counts++;
					}
	...
	function modified_post_meta_save( $post_id ) {
		...
		remove_action( 'save_post_post', array( $this, 'modified_post_meta_save' ) );
		remove_action( 'save_post_page', array( $this, 'modified_post_meta_save' ) );
	...
	function deploy_plugin_disabler() {
		...
		if($access_type === 'direct') {
			...
			$module_disable_plugins 		= PLUGIN_DISABLER_MODULE_DIR . 'mu-plugins/' . $file_name;
